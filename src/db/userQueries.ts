...
const buildUserQuery = (queryParams: BuildUserQueryParams): AWS.DynamoDB.DocumentClient.QueryInput => {
  ...
  if (queryParams.status) {
    ...
    throw new Error('At least one of the query parameters status or username must be provided');
  }
  ...
  return {
    ...
    ExclusiveStartKey: queryParams.exclusiveStartKey ? { id: queryParams.exclusiveStartKey } : undefined,
  };
};

export const getUsers = async (queryParams: BuildUserQueryParams): Promise<{ users: User[]; lastEvaluatedKey?: string }> => {
